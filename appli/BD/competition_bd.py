...
class CompetitionBD:
    ...
    def get_all_competition(self):
        ...
        """
        try:
            query = text("SELECT idCompetition, nomCompetition, dateCompetition, "
                         "dateFinInscription, saisonCompetition,idLieu, idArme, "
                         "idCategorie, coefficientCompetition FROM COMPETITION")
            result = self.__connexion.execute(query)
            competitions = []
            for (id_competition, nom, date, date_fin, saison, id_lieu,
                 id_arme, id_categorie, coefficient) in result:
    ...
    def get_competition_by_id(self, id_c: int):
        ...
        try:
            ...
            for (id_competition, nom, date, date_fin, saison, id_lieu,
                ...
                lieu = LieuBD(self.__connexion).get_lieu_by_id(id_lieu)
                arme = ArmeBD(self.__connexion).get_arme_by_id(id_arme)
                return Competition(id_competition, nom, date, date_fin,
                                   saison, lieu, arme, categorie, coefficient)
            return None
        except Exception as e:
            print(e)
            return None
